...
my_varimp <- read.csv(str_glue("{datafolder_path}/Data_NL/Model_output/Variable_importance/Variable_importance_eval_190.csv"))
...
# Order the levels
lvl_order <- my_plot_data[order(group_mean, variable_mean)] %>% 
...
my_plot_data2 <- my_plot_data %>% 
  mutate(
    ...
    Variable_type = factor(Variable_type, 
                           levels = c("Climate", "Land-use", "Terrain", "Distance to", "Demographic", "Environmental"))
  )
...
ggsave(str_glue("C:/Users/gorte/Documents/Studie/Master/MSc STAGE/Outputs/Figure_varimp_test.jpg"), p_varimp,
