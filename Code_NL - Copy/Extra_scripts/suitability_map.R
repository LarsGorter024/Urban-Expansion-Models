...
fullres_layers <- raster("/vol/milkunarc/ES_students/lgorter/Urban_Expansion_model/Data/Data_Global/Cropland/Agri_suitability_mosaic_10.tif")
  
r <- raster("/vol/milkunarc/ES_students/lgorter/Urban_Expansion_model/Data/Data_Global/Cropland/Agri_suitability_aggregated_10m_avg_10.tif")

r_5m <- raster("/vol/milkunarc/ES_students/lgorter/Urban_Expansion_model/Data/Data_Global/Cropland/Agri_suitability_aggregated_5m_avg_10.tif")
...
my_ext1 <- extent(c(112, 114, -1, 1))
my_ext2 <- extent(c(-61, -57, -19, -15))
my_ext3 <- extent(c(3.360782, 7.227095, 50.723492,53.554584))


my_poly_borneo <- my_ext1 %>% 
  st_bbox() %>% 
  st_as_sfc() %>% 
  st_set_crs(st_crs(4326))

my_poly_chaco <- my_ext2 %>% 
  st_bbox() %>% 
  st_as_sfc() %>% 
  st_set_crs(st_crs(4326))
...
r_borneo <- crop(fullres_layers, my_ext1)

r_chaco <- crop(fullres_layers, my_ext2)

r_NL <- crop(fullres_layers, my_ext3)

##

inset_gg_borneo <- raster_to_gg(r_borneo) %>% 
  rename(layer = 1)

p_inset_borneo <- ggplot() +
  geom_tile(data = inset_gg_borneo, aes(x = x, y = y, fill = layer)) +
  scale_fill_carto_c(
    palette = "SunsetDark", limits = c(0, 1)
  ) +
  coord_sf(
...
inset_gg_chaco <- raster_to_gg(r_chaco) %>% 
  rename(layer = 1)

p_inset_chaco <- ggplot() +
  geom_tile(data = inset_gg_chaco, aes(x = x, y = y, fill = layer)) +
  scale_fill_carto_c(
    palette = "SunsetDark", limits = c(0, 1)
  ) +
  coord_sf(
...
p <- ggplot() +
  ...
  geom_sf(data = my_poly_chaco, fill = NA, color = "red", size = 1) +
  geom_sf(data = my_poly_borneo, fill = NA, color = "purple", size = 1) +
...
bottom_part <- (p_inset_borneo | p_inset_chaco | p_inset_NL)  + 
  plot_layout(
    widths = unit(c(13, 13, 13), c("cm", "cm", "cm"))
...
##########
left_side <- ((p_inset_borneo | p_inset_chaco | p_inset_NL) / my_legend)  + 
  plot_layout(
...
p_inset_borneo_10 <- make_inset("asia", "10", frame_color = "purple")
  

ggsave(p_inset_borneo_10)

ggsave(str_glue("C:/Users/gorte/Documents/Studie/Master/MSc STAGE/Outputs/inset_cat10_borneo.png"), p_inset_borneo_10, dpi = 150,
       height = 14, width = 14)
