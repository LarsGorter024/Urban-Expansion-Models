...
evaluate_ann <- file.path(str_glue("{folder_path}/Code_NL/evaluate_ann.R"))
source(evaluate_ann)

variable_importance <- file.path(str_glue("{folder_path}/Code_NL/variable_importance.R"))
...
model_outfolder_eval <- str_glue("{datafolder_path}/Data_NL/Model_output/Evaluations")


# This is folder with response variable with extracted predictor values
mdata_folder <- str_glue("{datafolder_path}/Data_NL/Response_variable/Changes_extracted/Combined_files")
...
for (type in types)
{
  ...
  hyperparameters_filename <- str_glue("{model_outfolder}/Model_parameters_{category_id}.rds") %>%
  ...
  if (type == "fit")
  {
    ...
    if (!file.exists(model_outname_fit))
    {
      ...
      set.seed(666)
      # 
      model_data <- model_data_raw[,c(-3, -5, -6)]   %>%        #[,c(1,2,4,5,6,14,17)] %>%  # 3 is an excessive grid column , 12 KBA, 15 PPAs, 16 PAs
      ...
      colnames(model_data) <- c("PA", substring(names(model_data[,-1]),2))
      ...
      ###
      # Variable importance
      modvarimp_fname_fit <- str_glue("{datafolder_path}/Data_NL/Model_output/Variable_importance/Variable_importance_{category_id}.csv")
  ...
  if (type == "eval")
  {
    ...
    if (!file.exists(model_outname_eval))
    {
      ...
      set.seed(666)
      model_fit_data <- model_data_fit_raw[,c(-3, -5, -6)]  %>%         #[,c(1,2,4,5,6,14,17)] %>%   # 3 is an excessive grid column , 12 KBA, 15 PPAs, 16 PAs
        mutate_at(vars(contains("ESA")), ~ifelse(is.na(.), 0, 1)) %>% 
        drop_na() %>% 
        as_tibble() %>% 
        mutate(
      ...
      colnames(model_fit_data) <- c("PA", substring(names(model_fit_data[,-1]),2))
      
      set.seed(666)
      model_eval_data <- model_data_eval_raw[,c(-3, -5, -6)]  %>%         #[,c(1,2,4,5,6,14,17)] %>%   # 3 is an excessive grid column , 12 KBA, 15 PPAs, 16 PAs
        mutate_at(vars(contains("ESA")), ~ifelse(is.na(.), 0, 1)) %>% 
        drop_na() %>% 
        as_tibble() %>% 
        mutate(
      ...
      colnames(model_eval_data) <- c("PA", substring(names(model_eval_data[,-1]),2))
      ...
      my_variable_importance <- variable_importance(data = model_data_train, 
                                                    ...
                                                    iterations_num = 100,
                                                    clean = set_clean)
      print("my_variable_importance for eval")
      print(my_variable_importance)
