...
SAGA_distance <- function(infile, outfile, to_tiff = TRUE, ext)
{
...
# Reclassified
outname_190 <- str_glue("{datafolder_path}/Data_NL/Predictors_intermediate/ESA_190.tif") 


# Distance names
outname_190_distance <- str_glue("{datafolder_path}/Data_NL/Predictors_intermediate/ESA_190_distance.tif")


# Reclassify land cover rasters -------------------------------------------

# Reclassify 190 & correct for latitude with function
# # Temporary Projection !!! (Amersfoort rd projection)
# projectRaster(esa_lc_2005,crs="+proj=sterea +lat_0=52.15616055555555 +lon_0=5.38763888888889 +k=0.9999079 +x_0=155000 +y_0=463000 +ellps=bessel +towgs84=565.417,50.3319,465.552,-0.398957,0.343988,-1.8774,4.0725 +units=m +no_defs") %>%
#   distance() %>%
...
# 
# 
# outname_190_ea <- str_glue("{datafolder_path}/Data_NL/Predictors_intermediate/ESA_190_eqa2.tif") 
# 
...
#   SAGA_distance(outname_190_ea, outname_190_distance2, to_tiff = TRUE)
#   toc()
#   
# 
# }
...
{
  tic("Calculating distance 190")
  SAGA_distance(outname_190, outname_190_distance, to_tiff = TRUE)
  toc()
...
ESA_190_distance_resample <- resample(raster(outname_190_distance), dem,"bilinear")


writeRaster(ESA_190_distance_resample, filename = str_glue("{datafolder_path}/Data_NL/Predictors_intermediate/ESA_190_distance_resample.tif"), format="GTiff",overwrite=TRUE)

ESA_190_distance_fnl <- str_glue("{datafolder_path}/Data_NL/Predictors_final/ESA_190_distance_fnl.tif")

ESA_190_distance_fnl <- Crop_function(infile = str_glue("{datafolder_path}/Data_NL/Predictors_intermediate/ESA_190_distance_resample.tif"), 
                         outfile = str_glue("{datafolder_path}/Data_NL/Predictors_final/ESA_190_distance_fnl.tif"), 
                         scale = "NL")

# ESA_190_test <- raster(str_glue("{datafolder_path}/Data_NL/Predictors_final/ESA_190_distance_fnl.tif"))
# plot(ESA_190_test)
