...
pacman::p_load(Rahat, tidyverse, raster, foreach, tictoc, sf, rgdal, mapview, GSIF, foreach, doParallel,gdalR,mapview, glue)
...
reclassify_gdal <- file.path(str_glue("{folder_path}/Code_NL/GDAL/reclassify_vals_gdal.R"))
...
# "Data/Data_NL/Absences/" %>% 
#   dir.create(recursive = TRUE)


typefolder_path <- str_glue("{datafolder_path}/Data_NL/Absences/{changes_type}")
...
tempfolder_path <- str_glue("{datafolder_path}/Data_NL/Absences/temp")
...
base_folder <- str_glue("{datafolder_path}/Data_NL/Response_variable/Changes_vector")
...
absences <- raster::sampleRandom(change_raster, (abs_number*1.5), na.rm = TRUE, sp = TRUE)
toc()
