...
pacman::p_load(raster, rgdal, Rahat, tictoc, sf, fs, glue, tidyverse, pkgmaker, mapview, GSIF, foreach, doParallel, doSNOW, gdalR)
...
# Crop input rasters ------------------------------------------------------
lc_1993 <- raster(str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_1993_crop.tif"))
lc_2006 <- raster(str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_2006_crop.tif"))
lc_2019 <- raster(str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_2019_crop.tif"))

Crop_function(infile = str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_1993_crop.tif"), 
              outfile =  str_glue("{datafolder_path}/Data_{scale}/Landcover_maps/landcover_1993_crop.tif"), 
              scale = scale)

Crop_function(infile = str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_2006_crop.tif"), 
              outfile =  str_glue("{datafolder_path}/Data_{scale}/Landcover_maps/landcover_2006_crop.tif"), 
              scale = scale)

Crop_function(infile = str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_2019_crop.tif"), 
              outfile =  str_glue("{datafolder_path}/Data_{scale}/Landcover_maps/landcover_2019_crop.tif"), 
              scale = scale)

# Mask to which point data is rarified
# e <- extent(raster(str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_2019_crop.tif")))
# 
# bioclim_mask <- raster("//milkunstud-srv.science.ru.nl/milkundata/Chelsa/CHELSA_bioclim/CHELSA_bio10_1.tif") 
# 
# bioclim_mask_crop <- crop(bioclim_mask,esa_2019)
# writeRaster(bioclim_mask_crop, filename = str_glue("{datafolder_path}/bioclim_crop.tif"), format="GTiff", overwrite=TRUE)

# Crop_function(infile = str_glue("{datafolder_path}/bioclim_crop.tif"), 
...
# esa_2019 <- raster(str_glue("{datafolder_path}/Data_Global/Landcover_maps/lc_2019_crop.tif"))
# plot(esa_2019)
# 
# plot(bioclim_mask_crop)
# plot(raster(infile))
# plot(bioclim_mask)
...
{
  folder_basepath <- str_glue("{datafolder_path}/Data_{scale}/Response_variable/Changes_vector/{changes_type}/Presences") 
  ...
  get_change_raster_GDAL <- file.path(str_glue("{folder_path}/Code_{scale}/GDAL/get_change_raster_GDAL.R"))
  ...
  Crop_function(infile = str_glue("{folder_basepath}/Presence_{tolower(changes_type)}_190.tif"), 
                outfile =  str_glue("{datafolder_path}/Data_{scale}/Response_variable/Changes_vector/{changes_type}/Presences/Change_raster_{changes_type}.tif"), 
                scale = scale)
  
  change_raster <-  raster(str_glue("{datafolder_path}/Data_{scale}/Response_variable/Changes_vector/{changes_type}/Presences/Change_raster_{changes_type}.tif"))
  
  
  toc()
  
  print(freq(change_raster))
  ...
  rarify_points <- file.path(str_glue("{folder_path}/Code_{scale}/rarify_points.R"))
