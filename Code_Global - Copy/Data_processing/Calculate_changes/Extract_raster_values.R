...
pacman::p_load(Rahat, tidyverse, raster, sf, tictoc, glue)
...
predictors_list <- str_glue("{datafolder_path}/Data_Global/Predictors_final") %>%
  list.files(full.names = TRUE)

# Define folder in which the outputs will be stored
output_folder <- str_glue("{datafolder_path}/Data_Global/Response_variable") 
...
for (type in c("fit", "eval"))
{
  ...
  predictors_list_clean <- predictors_list %>%
    ...
    str_subset("ESA", negate = TRUE) %>%
    str_subset("Pop", negate = TRUE) %>%
    str_subset("Access", negate = TRUE) %>%
    str_subset("DEM", negate = TRUE) %>%
    str_subset("Proposed", negate = TRUE) %>%
    str_subset("Protected", negate = TRUE) %>%
    str_subset("KBAs", negate = TRUE) %>%
  ...
  out_layer_name <- predictors_list_clean[i] %>%
    str_remove(str_glue("{datafolder_path}/Data_Global/Predictors_final/")) %>%
    str_remove("_fnl.tif")
  ...
  #
  urbanchanges_path <- str_glue("/vol/milkunarc/ES_students/lgorter/Urban_Expansion_model/Data/Data_Global/Response_variable/Changes_vector/Urbanchanges_{category_id}_ppa_grids.shp")
