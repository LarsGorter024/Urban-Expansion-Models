...
# Input files
extracted_files_list <- str_glue("{datafolder_path}/Data_Global/Response_variable/Changes_vector/Single_files") %>% 
  list.files(pattern = "Urbanchanges.*ppa_grids.shp$", recursive = TRUE, full.names = TRUE)


####
# Folder for extracted files (output folder)
folder_extracted <- str_glue("{datafolder_path}/Data_Global/Response_variable/Changes_extracted")

for (file in rev(extracted_files_list))
{
  layer_name_temp <- file %>% 
    str_remove(str_glue("{datafolder_path}/Data_Global/Response_variable/Changes_vector/Single_files/")) %>% 
