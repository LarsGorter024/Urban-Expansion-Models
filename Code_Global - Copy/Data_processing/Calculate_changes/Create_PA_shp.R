...
grid_mask_filename <-  "/vol/milkundata/ESA_landcover/Version_2_1/C3S-LC-L4-LCCS-Map-300m-P1Y-2019-v2.1.1.tif"
...
for (type in c("fit", "eval"))
{
  ...
  if (!file.exists(urbanchanges_filename))
  {
    ...
    presence_files <- str_glue("{datafolder_path}/Data_Global/Response_variable/Changes_vector/{type}/Presences") %>% 
    ...
    absence_files <- str_glue("{datafolder_path}/Data_Global/Absences/{type}/Absences_{type}.gpkg") 
