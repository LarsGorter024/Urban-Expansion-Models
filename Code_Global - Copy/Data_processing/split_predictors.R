...
pacman::p_load(Rahat, tidyverse, raster, tictoc, gdalR, GSIF, rgdal)
...
my_layers <- str_glue("{datafolder_path}/Data_Global/Predictors_final") %>% 
...
for (tile in 1:nrow(tiles))
{
  ...
  output_foldername <- str_glue("{datafolder_path}/Data_Global/Predictors_splitted/{lyr_name}")
